#!/usr/bin/env python3
"""
Extract screenshot panels from Images.docx, composite with banner, save to images/ss/.
Layout per output image: banner at top + 3 panels in columns (natural sizes, no upscaling).
"""

import os, re, io, zipfile
from PIL import Image

DOCX    = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "temp", "Images.docx")
OUT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "images", "ss")
...
def main():
    ...
    with zipfile.ZipFile(DOCX) as z:
        ...
        banner = Image.open(io.BytesIO(z.read("word/media/image13.png"))).convert("RGBA")
